...
def spectr_to_spatial(qr, size_WM):
    ...
    qr_back[100 + 1:100 + 66, 100 + 1:100 + 33] = qr[:, :32]
    for i in range(65):
        for j in range(32):
            # print(qr_back[i + 1, j + 1], (100+i + 1, 100+ j + 1), (-100- (i + 1),- 100- (j + 1)),
            #       (256-(100 + i + 1),256-(100 + j + 1)))
            qr_back[-100 - (i + 1), -100 - (j + 1)] = np.conj(qr_back[100 + i + 1, 100 + j + 1])

    qr_back[100+1:100+66, -100-33:-100] = qr[:, 32:]
    # print(qr_back[1:66, -33:].shape, qr[:, 32:].shape)
    for i in range(66):
        for j in range(1, 34):
            qr_back[-100-(i + 1),100+ j] = np.conj(qr_back[100+i + 1, -100-j])
...
# img1.save(r"data/spatial_spectr_in_center_wm_65.png")
