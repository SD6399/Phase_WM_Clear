...
if __name__ == '__main__':
    ...
    file_path = EEG_ROOT + 'S001/S001R01.edf'

    # 1. Загружаем данные через MNE (он использует pyedflib внутри)
    raw = mne.io.read_raw_edf(file_path, preload=True)

    # 2. Чистим названия каналов от точек (важно для сопоставления!)
    # Ваши каналы выглядят как 'Fc5.', а в базе они 'Fc5'
    new_names = {name: name.strip('.') for name in raw.ch_names}
    raw.rename_channels(new_names)

    print("Каналы после очистки:", raw.ch_names[:10])
    # Read all signal data into a numpy array

    # Создаем стандартный монтаж 10-20 системы
    montage = mne.channels.make_standard_montage('standard_1020')

    # Применяем монтаж к данным
    # MNE автоматически сопоставит названия каналов с координатами
    raw.set_montage(montage, on_missing='warn')

    # Проверяем, сколько каналов удалось привязать к 3D
    coords = raw.get_montage().get_positions()
    print(f"3D координаты получены для {len(coords['ch_pos'])} каналов")
    ...
    for vid_name in ["cut_RealBarca120", "IndiDance", "Road"]:
        count = read_video(fr'D:/pythonProject/phase_wm/{vid_name}.mp4',
                           input_folder, total_count)
        embed(input_folder, output_folder, PATH_IMG, ampl, teta, 0)
        for rand_k in [0, 25, 50, 100]:
            ...
            base_filename = f'D:/pythonProject/Phase_WM_Clear/spector_random_shift_0_ampl{ampl}_{vid_name}mpeg2-mjpg_alf_0005'
            with (open(f'{base_filename}_vot_sp_final.txt', 'a') as f_vot, open(f'{base_filename}_stop_kadr.txt',
                                                                                'a') as f_stop):
                f_vot.write(f"rand_k={rand_k}: {vot_sp}\n")
                f_stop.write(f"rand_k={rand_k}: {stop_kadr1}\n")
            if not os.path.exists(f'{base_filename}__stop_kadr.txt'):
